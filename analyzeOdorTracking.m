...
%%% Filtering trajectory data
sam_freq = 100;
cutoff_freq = 30;
%%% Bin limits
SourceDistBinLimit = 8;
OdorAxisBinLimit = 4;
RectBinLimit = 8;
...
%%% Angular velocity upsample = 5 times sampling rate
angvel_upsample = 500;
%%% Angular velocity freq cutoff = 15 Hz (from Tammero and Dickinson)
angvel_cutoff_freq = 10;
%%% Angular velocity amplitude cutoff = 500 deg/s (based on Tammero and Dickinson)
angvel_amplitude_cutoff = 300;

%%% Print defaults
disp(strcat('Sampling Rate (fps):',num2str(sam_freq)));
disp(strcat('CutOff Freq (Hz):',num2str(cutoff_freq)));
disp(strcat('Angvel: Upsampling Rate (fps):',num2str(angvel_upsample)));
disp(strcat('Angvel: CutOff Freq (Hz):',num2str(angvel_cutoff_freq)));
disp(strcat('Angvel: CutOff Amplitude (deg/s):',num2str(angvel_amplitude_cutoff)));
fprintf('\n\n');
...
for m = 1:num
    ...
    data.angvel_upsample = angvel_upsample;
    data.angvel_cutoff_freq = angvel_cutoff_freq;
    data.angvel_amplitude_cutoff = angvel_amplitude_cutoff;  
    
    %%% Bin limits
    data.binLimits.SourceDistBin = SourceDistBinLimit;
    data.binLimits.OdorAxisBin = OdorAxisBinLimit;
    data.binLimits.RectBin = RectBinLimit;
    ...
    pooled_data.saccade_interval.trajData = cell(sel,1);   
    pooled_data.turn_interval.trajData = cell(sel,1);   
    pooled_data.saccade_interval_no_hover.trajData = cell(sel,1);   
    pooled_data.turn_interval_no_hover.trajData = cell(sel,1);   
    ...
    for i=1:1:sel
        ...
        % Add Hover to raw parameters
        data.trajData{i,1}.globalpara.Hover = (data.trajData{i,1}.globalpara.FlySpeed<=HoverSpeed);
        
        % Angular velocity calculation
        [data.trajData{i,1}.globalpara.AngVel,...
            data.trajData{i,1}.globalpara.TurnAng,...
            data.trajData{i,1}.globalpara.Saccades,...
            data.trajData{i,1}.globalpara.Turns,...
            data.trajData{i,1}.globalpara.T_AngVel,...
            data.trajData{i,1}.AngVel_Info] = calculateAngularVelocity(...
            data.trajData{i,1}.expdata.pts,data.sam_freq,...
            data.angvel_upsample,data.angvel_cutoff_freq,...
            data.angvel_amplitude_cutoff);
        ...
        data.trajData{i,1}.OverallSum.Saccades = sum(data.trajData{i,1}.globalpara.Saccades);
        data.trajData{i,1}.OverallSum.Turns = sum(data.trajData{i,1}.globalpara.Turns);
        
        % Add intersaccade interval and interturn interval
        [pooled_data.saccade_interval.trajData{i,1},...
            pooled_data.turn_interval.trajData{i,1},...
            pooled_data.saccade_interval_no_hover.trajData{i,1},...
            pooled_data.turn_interval_no_hover.trajData{i,1}] = ...
            calculateInterSaccadeInterval(...
            data.trajData{i,1}.globalpara.Saccades,...
            data.trajData{i,1}.globalpara.Turns,...
            data.trajData{i,1}.globalpara.Hover,data.sam_freq);
        
        % Add to overall average
        data.trajData{i,1}.OverallAverage.SaccadeInterval = ...
            nanmean(pooled_data.saccade_interval.trajData{i,1});
        data.trajData{i,1}.OverallAverage.TurnInterval = ...
            nanmean(pooled_data.turn_interval.trajData{i,1});
        data.trajData{i,1}.OverallAverage.SaccadeIntervalNoHover = ...
            nanmean(pooled_data.saccade_interval_no_hover.trajData{i,1});
        data.trajData{i,1}.OverallAverage.TurnIntervalNoHover = ...
            nanmean(pooled_data.turn_interval_no_hover.trajData{i,1});        
        ...
        for j=1:size(binNames(1:end-1),1)            
            ...
            % Addition: 29/03/2017 - Saccades
            data.sortedBins.(char(binNames(j))).Saccades(:,i) =  data.trajData{i,1}.binnedData.(char(binNames(j))).Saccades.zoneSum; % Total saccades per bin 
            data.sortedBins.(char(binNames(j))).Turns(:,i) =  data.trajData{i,1}.binnedData.(char(binNames(j))).Turns.zoneSum; % Total turns per bin 
        end
...
%         [data.trajData{i,1}.expPara.vAngDeg,data.trajData{i,1}.expPara.vAngRad] = visualAngle(data.trajData{i,1}.expdata.pts,data.trajData{i,1}.expdata.cs);
    ...
    pooled_assay.(strcat('t',data.treatment)) = pooled_data;
...
save(fullfile(destpath, strcat('analyzeddata','_cutoff',...
        num2str(cutoffFreq), '.mat')),'assay','pooled_assay');
