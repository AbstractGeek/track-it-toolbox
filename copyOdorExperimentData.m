...
inputDir_pref = '/home/dinesh/Desktop/Track-it-test';
outputDir_pref = '/home/dinesh/Desktop/Track-it-test';
moveDir_pref = '/home/dinesh/Desktop/Track-it-test/move';

%% Let's rock and roll?
% Get the experiment folder
expDir = uigetdir(inputDir_pref,'Experiment Folder(OneDayOnly-Please)');
...
% Obtain the Save/Output directory
outDir = uigetdir(outputDir_pref,'Output Folder');
csvDir = fullfile(outDir,'ExpCSVFiles');
if ~isdir(csvDir)
    mkdir(csvDir);
end

[tags,datenums,varnames,trialnames] = decipherTrialNamesAndTags (dirList);
time = datestr(datenums);
expname = datestr(datenums(1),'yyyy_mm_dd');
attributes = table(varnames,tags,time,datenums);


%% Begin by copying the headers
fid=fopen(fullfile(expDir,dirList{1},strcat('Filtered_',trialnames{1},'.csv')));
headerline=fgetl(fid);
data.colheaders=textscan(headerline,'%s','delimiter',';');
data.colheaders=data.colheaders{1}';
fclose(fid);
...
for i=1:dirNum
    ...
    currDir = dirList{i};
    currFile = fullfile(expDir,currDir,strcat('Filtered_',trialnames{i},'.csv'));
    trackItData = txt2mat(currFile);
    if ~isempty(trackItData)
        data.(varnames{i}) = trackItData;
        % Data import done
        % Copy csv files to the output location
        copyfile(currFile,csvDir);
    else
        disp(strcat('Ignored:',currDir));
    end
    % Done, loop around.
end
close(h);
% Save mat file
 data.(strcat('attr_',expname)) = attributes;
save(fullfile(outDir,expname),'-struct','data');
% Done saving
% Delete vars to save memory
clearvars data trackItData;

% Extract trajectories
extractTrajectories(fullfile(outDir,expname));


% Move folders to backup location now - if required
choice = questdlg('Would like to move your raw experimental files into a backup location?', 'Move Raw Files?'...
    ,'Yes','No','Maybe','Maybe');
while (strcmp(choice,'Maybe'))
    choice = questdlg('Would like to move your raw experimental files into a backup location?', 'Move Raw Files?'...
        ,'Yes','No','Maybe','Maybe');
end
...
% Obtain Move directory
movDir = uigetdir(moveDir_pref,'Output Folder');

% Start moving files
% open a waitbar
h = waitbar((1-1)/(dirNum-1),strcat('Moving: Trial ',num2str(1),'-',num2str(dirNum),'...'));
pos = get(h,'Position');
set(h,'Position',[pos(1) pos(2)+2.*pos(4) pos(3) pos(4)]);

for i=1:dirNum
    waitbar((i-1)/(dirNum-1),h,strcat('Moving: Trial ',num2str(i),'-',num2str(dirNum),'...'));
    currDir = dirList{i};
    movefile(fullfile(expDir,currDir),movDir);
end
close(h);
% Done
% Exit
...
%%% Obtained from the txt2mat code %%%
%%%
%%%
...
if doAnalyzeFile
    ...
    if ~isempty(aerrmsg)
        ...
        hl = ana_hl;
        if infolvl>=1
            % Modification to stop displaying - Dinesh
    ...
    % display information:
    % Modification to stop function from printing on screen - Dinesh
    %     if infolvl >= 1
